...
<div class="main-container container-fluid">
    <div class="page-header">
        <h1 class="page-title">Saran Dan Tanggapan</h1>
        <div>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('superadmin.home.index') }}">List</a></li>
                <li class="breadcrumb-item active" aria-current="page">Saran Dan Tanggapan</li>
            </ol>
        </div>
    </div>
</div>
...
<div class="row justify-content-center mt-4">
    ...
    <div class="col-md-12 ">
        @foreach ($saran as $sarans)
        <div class="modal fade effect-scale show" id="confirmDeleteModal{{ $sarans->id }}" aria-modal="true"
            role="dialog">
                ...
                <div class="modal-content modal-content-demo">
                    ...
                    <div class="modal-footer">
                        <a href="javascript:void(0)" class="btn btn-white"
                            onclick="hideCustomAlert({{ $sarans->id }})">Cancel</a>
                        <form action="{{ route('admin.saran.destroy', ['id' => $sarans->id]) }}" method="POST">
                            @csrf
                            @method('DELETE')
                    ...
                    <table class="table table-bordered border text-nowrap mb-0" id="basic-edit">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $sarans->email }}</td>
                                <td>{{ $sarans->pesan }}</td>
                                <td>{{ \Carbon\Carbon::parse($sarans->created_at)->isoFormat('H:mm, dddd D MMMM YYYY') }}</td>
                                <td name="bstable-actions">
                                    <div class="btn-list">
                                        <button id="bDel" type="button" class="btn  btn-sm btn-danger"
                                            onclick="showCustomAlert({{ $sarans->id }})"> <span class="fe fe-trash-2">
                                            </span>
                                        </button>
                                    </div>
                                </td>
