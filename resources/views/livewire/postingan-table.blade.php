...
<input wire:model="search" wire:keydown="triggerSearch"
    ...
    <div class="flex mx-auto mt-5 justify-center">
        @if ($search)
        <p class="text-xs">Mencari di antara {{ $totalPostingan }} Postingan dengan Waktu eksekusi pencarian: {{
            number_format($executionTime, 5, ',', '') }} detik atau {{ number_format($executionTime, 1, '.', ',') }} detik</p>
        @endif
    </div>
